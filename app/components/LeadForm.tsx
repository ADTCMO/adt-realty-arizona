...
export function LeadForm({ type, stage }: { type: "seller" | "contact" | "career"; stage?: string }) {
  ...
  if (type === "seller") return <form className="compactForm" onSubmit={submit}><input type="hidden" name="leadType" value="Seller home value"/><label>Property Address<input name="address" autoComplete="street-address" placeholder="Enter your property address" required/></label><label>Email Address<input name="email" type="email" autoComplete="email" placeholder="you@example.com" required/></label><button className="button red" type="submit">Get My Home Value <span>›</span></button><small>No obligation. Your information stays private.</small><p className="formStatus" role="status">{status}</p></form>;
