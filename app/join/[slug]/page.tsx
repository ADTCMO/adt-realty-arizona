...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="guidePage"><header className="guideHeader"><Link href="/#join">‹ Back to career stages</Link><Link href="/"><img src="/adt-realty-arizona-outline.png" alt="ADT Realty Arizona"/></Link></header><section><p className="eyebrow">ADT Realty Career Guide</p><h1>{guide.title}</h1><div className="canvaEmbed"><iframe loading="lazy" src={guide.embed} allowFullScreen title={`${guide.title} guide`} /></div><p className="embedFallback">Having trouble viewing the guide? <a href={guide.publicUrl} target="_blank" rel="noreferrer">Open it in Canva</a>.</p></section></main>;
}
